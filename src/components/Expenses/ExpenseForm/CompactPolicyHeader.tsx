    ...
    <div className="mb-4">
      <Dialog>
        <DialogTrigger asChild>
          <Button
            variant="ghost"
            className="w-full bg-gradient-to-r from-red-50 to-red-100 hover:from-red-100 hover:to-red-200 
            text-red-700 gap-2 rounded-xl shadow-sm transition-all duration-200 hover:scale-[1.01] px-4 py-2"
          >
            <Flag className="h-4 w-4" />
            <span className="font-medium">{totalViolations} Policy {totalViolations === 1 ? 'Violation' : 'Violations'} flagged</span>
            <span className="text-xs underline ml-1">Tap to review</span>
          </Button>
        </DialogTrigger>
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="flex gap-4 justify-center pt-2">
                <Button 
                  ...
                  className="flex-1 gap-2 px-6 py-8 h-auto bg-blue-50 hover:bg-blue-100 text-blue-700 rounded-xl hover:scale-[1.02] transition-transform shadow-sm"
                >
                  <span className="text-2xl">👤</span>
                  <span className="font-medium">I'm the Initiator</span>
                </Button>
                <Button 
                  onClick={() => handleRoleSelection('approver')}
                  className="flex-1 gap-2 px-6 py-8 h-auto bg-gray-50 hover:bg-gray-100 text-gray-700 rounded-xl hover:scale-[1.02] transition-transform shadow-sm"
                >
                  <span className="text-2xl">🧑‍💼</span>
            ...
            <>
              <DialogHeader className="p-6 pb-2 border-b">
                ...
                <p className="text-sm text-gray-500 mt-1">
                  {reviewerRole === 'user' 
                    ? "Please review and justify the flagged violations below" 
                    : "Review justifications and provide feedback if needed"}
                </p>
              </DialogHeader>
              <div className="p-4">
                <PolicyViolationsHeader
                  violations={violations}
                  onAddComment={(violationId, comment, type) => onAddComment(violationId, comment, type || reviewerRole)}
                />
              </div>
              <div className="flex justify-end gap-2 p-4 border-t bg-gray-50">
                <Button 
                  variant="outline" 
                  onClick={() => setReviewerRole(null)}
