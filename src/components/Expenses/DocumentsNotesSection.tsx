  ...
  return <div className="mt-6 grid grid-cols-1 lg:grid-cols-2 gap-6">
      <div className="">
        ...
        <div className="space-y-2 mb-4">
          {uploadedDocuments.length > 0 ? uploadedDocuments.map((doc, index) => <div key={index} className="flex items-center justify-between bg-gray-50 p-2 rounded border text-sm">
                ...
                <button onClick={() => handleRemoveDocument(index)} className="text-gray-400 hover:text-gray-600">
                  <X className="h-4 w-4" />
                </button>
              </div>) : <div className="text-center p-8 border-2 border-dashed rounded-lg bg-gray-50">
      ...
      <NotesSection notes={notes} setNotes={setNotes} showPolicyText={true} policyTextMaxLength={150} validation={fieldValidations.notes} />
    </div>;
};
